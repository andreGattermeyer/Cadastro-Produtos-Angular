    ...
    this.vendaService.buscarTodasVendas().subscribe(
      (data) => {
        ...
        this.vendas = data.map(venda => {
          // Se temos apenas o vendedorId mas não o objeto vendedor completo
          if (venda.vendedorId && !venda.vendedor) {
            ...
            if (vendedorEncontrado) {
              ...
              // Se não encontrar o vendedor, criar um objeto com informações básicas
              venda.vendedor = { 
                id: venda.vendedorId, 
                nome: `Vendedor ID: ${venda.vendedorId}` 
              };
            }
          } else if (!venda.vendedor) {
            // Se não tiver nem vendedor nem vendedorId
