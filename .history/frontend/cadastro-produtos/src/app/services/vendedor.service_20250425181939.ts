
import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class VendedorService {
  private apiUrl = 'http://localhost:8080/api/vendedores';

  constructor(private http: HttpClient) {}

  // POST: Salvar vendedor
  salvarVendedor(vendedor: any) {
    console.log('Enviando vendedor para API:', vendedor);
    // Tente usar o endpoint específico para salvar vendedor
    return this.http.post<any>('http://localhost:8080/api/vendedores/salvar', vendedor);
  }

  // GET: Listar todos os vendedores
  buscarTodosVendedores(): Observable<any[]> {
    return this.http.get<any[]>(this.apiUrl);
  }

  // GET: Buscar vendedor por ID
  buscarVendedorPorId(id: number): Observable<any> {
    return this.http.get(`${this.apiUrl}/${id}`);
  }

  // PUT: Atualizar vendedor
  atualizarVendedor(id: number, vendedor: any): Observable<any> {
    return this.http.put(`${this.apiUrl}/${id}`, vendedor);
  }

  // DELETE: Apagar vendedor
  deletarVendedor(id: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/${id}`);
  }

  // Armazenar temporariamente um vendedor para edição
  private vendedorParaEditar: any = null;

  setVendedorParaEditar(vendedor: any): void {
    this.vendedorParaEditar = vendedor;
  }

  getVendedorParaEditar(): any {
    return this.vendedorParaEditar;
  }
}

## Solução

Vamos corrigir o método `salvarVendedor` no seu serviço. Existem duas possibilidades:

1. O endpoint correto pode ser algo como `/api/vendedores/salvar` ou `/api/vendedores/gravar-vendedor`
2. O backend pode estar esperando um método HTTP diferente, como PUT

Vamos modificar o serviço para usar o endpoint correto:
```typescript
import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class VendedorService {
  private apiUrl = 'http://localhost:8080/api/vendedores';

  constructor(private http: HttpClient) {}

  // POST: Salvar vendedor
  salvarVendedor(vendedor: any) {
    console.log('Enviando vendedor para API:', vendedor);
    // Tente usar o endpoint específico para salvar vendedor
    return this.http.post<any>('http://localhost:8080/api/vendedores/salvar', vendedor);
  }

  // GET: Listar todos os vendedores
  buscarTodosVendedores(): Observable<any[]> {
    return this.http.get<any[]>(this.apiUrl);
  }

  // GET: Buscar vendedor por ID
  buscarVendedorPorId(id: number): Observable<any> {
    return this.http.get(`${this.apiUrl}/${id}`);
  }

  // PUT: Atualizar vendedor
  atualizarVendedor(id: number, vendedor: any): Observable<any> {
    return this.http.put(`${this.apiUrl}/${id}`, vendedor);
  }

  // DELETE: Apagar vendedor
  deletarVendedor(id: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/${id}`);
  }

  // Armazenar temporariamente um vendedor para edição
  private vendedorParaEditar: any = null;

  setVendedorParaEditar(vendedor: any): void {
    this.vendedorParaEditar = vendedor;
  }

  getVendedorParaEditar(): any {
    return this.vendedorParaEditar;
  }
}

## Solução

Vamos corrigir o método `salvarVendedor` no seu serviço. Existem duas possibilidades:

1. O endpoint correto pode ser algo como `/api/vendedores/salvar` ou `/api/vendedores/gravar-vendedor`
2. O backend pode estar esperando um método HTTP diferente, como PUT

Vamos modificar o serviço para usar o endpoint correto:
```typescript
import { Injectable } from '@angular/core';
import { HttpClient } from '@angular/common/http';
import { Observable } from 'rxjs';

@Injectable({
  providedIn: 'root'
})
export class VendedorService {
  private apiUrl = 'http://localhost:8080/api/vendedores';

  constructor(private http: HttpClient) {}

  // POST: Salvar vendedor
  salvarVendedor(vendedor: any) {
    console.log('Enviando vendedor para API:', vendedor);
    // Tente usar o endpoint específico para salvar vendedor
    return this.http.post<any>('http://localhost:8080/api/vendedores/salvar', vendedor);
  }

  // GET: Listar todos os vendedores
  buscarTodosVendedores(): Observable<any[]> {
    return this.http.get<any[]>(this.apiUrl);
  }

  // GET: Buscar vendedor por ID
  buscarVendedorPorId(id: number): Observable<any> {
    return this.http.get(`${this.apiUrl}/${id}`);
  }

  // PUT: Atualizar vendedor
  atualizarVendedor(id: number, vendedor: any): Observable<any> {
    return this.http.put(`${this.apiUrl}/${id}`, vendedor);
  }

  // DELETE: Apagar vendedor
  deletarVendedor(id: number): Observable<any> {
    return this.http.delete(`${this.apiUrl}/${id}`);
  }

  // Armazenar temporariamente um vendedor para edição
  private vendedorParaEditar: any = null;

  setVendedorParaEditar(vendedor: any): void {
    this.vendedorParaEditar = vendedor;
  }

  getVendedorParaEditar(): any {
    return this.vendedorParaEditar;
  }
}



