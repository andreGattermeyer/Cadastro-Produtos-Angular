  ...
  carregarVendas(): void {
    this.vendaService.buscarTodasVendas().subscribe(
      (data) => {
        ...
        if (data.length > 0) {
          // Imprimir todas as propriedades do primeiro item para análise
          console.log('Propriedades da venda:', Object.keys(data[0]));
          console.log('Estrutura completa do primeiro item:', JSON.stringify(data[0]));
        }

        // Processa as vendas para adicionar informações do vendedor
        this.vendas = data.map(venda => {
          // Verificar todas as possíveis propriedades relacionadas ao vendedor
          // Exemplo: vendedorId, vendedor_id, vendedor, etc.
          console.log('Analisando venda:', venda);

          // Se não encontrarmos nenhuma informação do vendedor, podemos
          // temporariamente associar um vendedor padrão ou mostrar "Não disponível"
          if (!venda.vendedor) {
            venda.vendedor = { nome: 'Não disponível' };
          }
