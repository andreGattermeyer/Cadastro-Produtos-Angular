...
export class ListaVendasComponent implements OnInit {
    ...
    this.vendaService.buscarTodasVendas().subscribe(
      (data) => {
        ...
        if (data.length > 0) {
          ...
          console.log('Vendedor do primeiro item:', data[0].vendedor);
        }

        // Processa as vendas para adicionar informações do vendedor
        this.vendas = data.map(venda => {
          // Se a venda tem um ID de vendedor mas não tem o objeto vendedor completo
          if (venda.vendedorId && (!venda.vendedor || !venda.vendedor.nome)) {
            // Procura o vendedor na lista de vendedores já carregada
            const vendedorEncontrado = this.vendedores.find(v => v.id === venda.vendedorId);
            if (vendedorEncontrado) {
              venda.vendedor = vendedorEncontrado;
            }
          }
          return venda;
        });
  ...
  // Métodos para registrar nova venda
