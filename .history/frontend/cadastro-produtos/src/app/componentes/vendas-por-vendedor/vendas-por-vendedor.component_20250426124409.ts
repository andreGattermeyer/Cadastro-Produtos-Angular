  ...
  carregarVendas(vendedorId: number): void {
    this.vendaService.buscarVendasPorVendedor(vendedorId).subscribe(
      (data) => {
        ...
        // Verificar se os dados do produto estão presentes
        if (this.vendas.length > 0) {
          console.log('Primeiro item da lista:', this.vendas[0]);
          if (!this.vendas[0].produto) {
            console.error('Produto não encontrado na venda');
          }
        }
      },
      (error) => {
