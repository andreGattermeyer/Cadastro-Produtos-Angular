...
export class RelatoriosVendasComponent implements OnInit {
    ...
    this.vendaService.buscarTodasVendas().subscribe(
      (data) => {
        ...
        if (data && data.length > 0) {
          console.log('Datas e vendedores das vendas existentes:');
          data.forEach(venda => {
            console.log(`Venda ID: ${venda.id}, Data: ${venda.dataVenda}, Vendedor ID: ${venda.vendedor?.id}, Produto: ${venda.produto?.nomeProduto}`);
          });
        }
  ...
  formatarDataInput(data: Date | null): string {
    if (!data) return '';
  ...
  // Método para exibir a data formatada no template
  get dataFimFormatada(): string {
    return this.formatarDataInput(this.dataFim);
  }
